...
export function ProductImages({
  ...
  return (
    <div className="flex w-full gap-2 h-full max-h-[800px] flex-col md:flex-row">
      {productImages && mainImage ? (
        <>
          <div className="flex flex-row md:flex-col gap-2 max-h-fit md:max-h-[600px] order-1 md:order-0">
            {productImages.map((image: any) => {
              return (
                <div
                  key={image.url}
                  className={`cursor-pointer ${
                    displayImage === image.url && "brightness-75"
                  }`}
                ...
                >
                  <div className="w-full h-full rounded-lg overflow-hidden">
                    <img
                      src={image.url}
                      alt="product image"
                      className="pointer-events-none rounded-lg w-[75px] h-auto md:w-[135px] md:h-auto object-contain"
                    />
                  </div>
                </div>
              );
          ...
          <div className="rounded-lg relative w-full h-full overflow-hidden order-0 md:order-1">
            <img
              src={displayImage}
              alt="product image"
